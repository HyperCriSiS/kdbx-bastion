...
6. Each asynchronous unlock/browse operation carries a monotonically increasing lifecycle epoch. Backgrounding or explicit lock invalidates the epoch and calls Rust `lock-all`; stale work cannot publish state, and a handle produced after staleness is immediately locked.
